...
public class SchemaBuilderTests
{
    ...
    public void testSchemaBuilderWithPartiallyInitializedMetadata()
    {
        ...
        // Cassandra 6.0 replaces Schema.transform with submit, which an offline caller must follow by creating the
        // keyspace instances; SchemaUpdater.submit does both
        SchemaUpdater.submit(Schema.instance, SchemaTransformations.addKeyspace(keyspaceMetadata, false));
        ...
        TableMetadata tableMetadata = CQLFragmentParser
                ...
                .prepare(null)
                // Cassandra 6.0 adds the user functions argument to CreateTableStatement.builder
                .builder(Types.none(), UserFunctions.none())
                .build();
        KeyspaceMetadata keyspace = Schema.instance.getKeyspaceMetadata(keyspaceName);
        // SchemaTransformation declares two methods in Cassandra 6.0, so a lambda no longer compiles
        SchemaUpdater.submit(Schema.instance, new SchemaTransformation()
        {
            @Override
            public Keyspaces apply(ClusterMetadata metadata)
            {
                return metadata.schema
                       .getKeyspaces()
                       .withAddedOrUpdated(keyspace.withSwapped(keyspace.tables.with(tableMetadata)));
            }
